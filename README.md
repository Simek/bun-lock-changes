...
Creates a comment inside Pull Request with the human-readable summary of the changes to the `bun.lock` file. Works in public and private repositories, offers a degree of customization.
...
| `path`                 |         No         | `bun.lock` | Path to the `bun.lock` file in the repository. Default value points to the file at project root.                  |
...
To ensure that sufficient permissions for this action are always granted, you will need to add `permissions` entry to the job which runs `bun-lock-changes`:
